...
main = do
  ...
  let inputs = lines f
  let vents = (filterDiagonalVents . parseCoords . map words) inputs
...
filterDiagonalVents :: [Coords] -> [Coords]
filterDiagonalVents = filter (\(x1,y1,x2,y2) -> x1 == x2 || y1 == y2)

calcCoords :: Coords -> [Coord]
calcCoords (x1,y1,x2,y2)
  | x1 == x2 = let ys = if y2 > y1 then [y1..y2] else [y2..y1] in [(x1,y3) | y3 <- ys]
  | y1 == y2 = let xs = if x2 > x1 then [x1..x2] else [x2..x1] in [(x3,y1) | x3 <- xs]
calcCoords (_,_,_,_) = []
